 ...
 * @param cmp_option            the car selection mode, which must be either SIMPLEX or RANDOM
 */
int update_ext_vec(point_set *P, point_set *realDataset, vector<int> &C_idx, point_t *u, int s, vector<point_t *> &ext_vec, int &current_best_idx,
               int &last_best, vector<int> &frame, int cmp_option, Partition *R, bool error, server* ss, websocketpp::connection_hdl hdl,
                    ...
                    std::ofstream &out_cp, bool EnglishChinese)
{
    ...
    */
    ...
    point_t* p1 = realDataset->points[P->points[C_idx[S[0]]]->id];
    point_t* p2 = realDataset->points[P->points[C_idx[S[1]]]->id];

    int compareResult = p1->interaction_compare(p1, p2, ss, hdl, client_info, out_cp, EnglishChinese);
                    ...
                    std::owner_less<websocketpp::connection_hdl>> &client_info, std::ofstream &out_cp, bool EnglishChinese)
{
    ...
    while (resultSet->points.size() < k)
    {
        ...
        while (C_idx.size() > 1) // while none of the stopping condition is true
        {
            ...
            int errorout;
            errorout = update_ext_vec(pSet, realDataset, C_idx, u, s, ext_vec, current_best_idx, last_best, frame,
                                      cmp_option, R, error, ss, hdl, client_info, out_cp, EnglishChinese);
            if(errorout == 888)
                return 888;
            //R->print();
            //std::cout << C_idx.size() << "\n";
    ...
    resultSet->resultEvaluation(realDataset, "UHSimplex", numOfQuestion, ss, hdl, client_info, out_cp, EnglishChinese);
    return numOfQuestion;
}
