...
public:
    ...
    void fairTopk_diverse_results(const int k, const std::vector<std::string> &category, point_set *topSet);
    void fairTopk_bound_diverse(const int k, const std::vector<std::string> &category, point_t *u, point_set *topSet);
    ...
    void printResult_DiversityComparison(char *name, int Qcount, const std::vector<std::string> &category,
                                         point_set* realResultSet, point_t *u, timeval t1, double preTime, long mem_baseline);

    int resultEvaluation(point_set* realDataset, char *name, int Qcount, server* s, websocketpp::connection_hdl hdl,
