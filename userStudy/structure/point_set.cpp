...
std::string cent(const std::string& str, int width)
{
...
int point_set::resultEvaluation(point_set* realDataset, char *name, int Qcount, server* s, websocketpp::connection_hdl hdl,
                                ...
                                std::ofstream &out_cp, bool englishChinese)
{
    ...
    std::string present, presentC;
    present += "Here are the recommended cars based on your respond. \n";
    present += "---------------------------------------------------------------------------------------------\n";
    ...
    for (int i = 0; i < points.size(); ++i)
    {
        ...
        ss2 << "|" << cent(std::to_string(points[i]->id), 10);
        ss2 << "|" << cent((std::stringstream() << std::fixed << std::setprecision(1) <<
            (realDataset->points[points[i]->id]->attr[0]/1000) << "k").str(), 19);

        ss2 << "|" << cent((std::stringstream() << std::fixed << std::setprecision(1) <<
            realDataset->points[points[i]->id]->attr[1]).str(), 19);

        ss2 << "|" << cent((std::stringstream() << std::fixed << std::setprecision(1) <<
              (realDataset->points[points[i]->id]->attr[2]/10000) << "k").str(), 19);
        if(points[i]->cateAttr == "AT")
            ss2 << "|" << cent("Automatic", 19);
        else if (points[i]->cateAttr == "MT")
            ss2 << "|" << cent("Manual", 19);
        else
            ss2 << "|" << cent("Others", 19);
        present += ss2.str();
    ...
    std::string satisfaction_present = present;
    satisfaction_present += "Please rate your satisfaction with the returned cars on a scale from 1 to 10 (i.e., 1, 2, .., 10). \n"
                            "Here, 1 means you are the least satisfied, and 10 means you are the most satisfied.";

    if(englishChinese == 0)
    {
        s->send(hdl, satisfaction_present, websocketpp::frame::opcode::text);
    } else
    {
        presentC += "以下是根据您的回答给您推荐的车\n";
        presentC += "---------------------------------------------------------------------------------------------\n";
        // 使用 stringstream 来格式化字符串
        ...
        for (int i = 0; i < points.size(); ++i)
        {
            ...
            ss5 << "|" << cent(std::to_string(points[i]->id), 10);
            ss5 << "|" << cent((std::stringstream() << std::fixed << std::setprecision(1) <<
                                                    (realDataset->points[points[i]->id]->attr[0]/1000) << "k").str(), 19);
        ...
        std::string satisfaction_presentC = presentC;
        satisfaction_presentC += "请按照 1 到 10 的评分标准来评估您对返回车辆的满意度(例如1, 2, ..., 10)。其中 1 表示最不满意，10 表示最满意。";
        s->send(hdl, satisfaction_presentC, websocketpp::frame::opcode::text);
    }

    std::string user_input;
    int satisfaction, boredness;
    // 阻塞等待用户输入或退出信号
    {
        ...
        satisfaction = std::stoi(user_input);
        while (satisfaction < 1 || satisfaction > 10)
        {
    ...
    std::string boredness_present = present;
    boredness_present += "\nThe number of questions you asked: ";
    boredness_present += std::to_string(Qcount);
    boredness_present += "\n\nPlease give a number from 1 to 10 (i.e., 1, 2, .., 10) to indicate how bored you feel when\n"
                         "you are asked with ";
    boredness_present += std::to_string(Qcount);
    boredness_present += " questions for this round in order to obtain these returned cars.\n"
                         "Here 10 denotes that you feel the most bored and 1 denotes that you feel the least bored.\n";

    if(englishChinese == 0)
        s->send(hdl, boredness_present, websocketpp::frame::opcode::text);
    else
    {
        std::string boredness_presentC = presentC;
        boredness_presentC += "\n这轮您被问的问题个数为: ";
        boredness_presentC += std::to_string(Qcount);
        boredness_presentC += "\n\n请根据问题个数和推荐的车辆，为本轮提问的无聊程度打分，范围是 1 到 10 (例如：1, 2, .., 10)\n";
        boredness_presentC += "注意：10 表示您感到最无聊，1 表示您感到最不无聊。\n";
        s->send(hdl, boredness_presentC, websocketpp::frame::opcode::text);
    }
    ...
    // 阻塞等待用户输入或退出信号
    {
        ...
        user_input = client_info[hdl].user_input;  // 获取输入
        boredness = std::stoi(user_input);
        while (boredness < 1 || boredness > 10)
        {
            client_info[hdl].data_ready = false;
            client_info[hdl].cv.wait(lock, [&]() { return client_info[hdl].data_ready || client_info[hdl].should_exit; });  // 等待输入或退出信号
    ...
    out_cp << present << "\n";
    out_cp << "questions: " << Qcount << "\n";
    out_cp << "satisfaction: " << satisfaction << "\n";
    out_cp << "boredness: " << boredness << "\n\n";
 ...
 * @param mem_baseline  The memory usage before the algorithm
 */
void point_set::printResult_DiversityComparison(char *name, int Qcount, const std::vector<std::string> &category, point_set* realResultSet, point_t *u, timeval t1, double preTime, long mem_baseline)
{
    timeval t2; gettimeofday(&t2, 0);
    double time_cost = (double) t2.tv_sec + (double) t2.tv_usec / 1000000 - (double) t1.tv_sec - (double) t1.tv_usec / 1000000;
    double accNum = this->PercentageOfSamePoint(realResultSet);
    ...
    double sumUtility = 0;
    for(int i = 0; i < points.size(); ++i)
        sumUtility += points[i]->dot_product(u);
    std::cout << "The sum of utilities: " << sumUtility << "\n";

    int *group_number = new int[category.size()];
    ...
    std::cout << "The number of points in each group: \n";
    for (int i = 0; i < category.size(); ++i)
        std::cout << "Group" << i+1 << ": " << group_number[i] << " \n";

    std::ofstream out_cp("../result.txt");
    out_cp << Qcount << "       " << time_cost - preTime <<"       " << sumUtility << "\n"; //<< get_mem_usage() - mem_baseline << "\n";
    for (int i = 0; i < category.size(); ++i)
        out_cp << group_number[i] << " \n";
}
 ...
 * @param topSet        The returned diverse top-k set
 */
void point_set::fairTopk_diverse_results(const int k, const std::vector<std::string> &category, point_set *topSet)
{
    std::vector<std::vector<double>> bound(category.size(), std::vector<double>(2));
    //Find the number of points that can be selected randomly
    double rest = 0;
    double categorySize = double(category.size());
    for(int i = 0; i < category.size(); ++i)
    {
        bound[i][0] = std::floor(k / categorySize);
        bound[i][1] = std::ceil(k / categorySize);
        rest += bound[i][0];
    }
    rest = k - rest;
    if(rest < 0)
...
void point_set::fairTopk_bound_diverse(const int k, const std::vector<std::string> &category, point_t *u, point_set *topSet)
{
    int *bound = new int[category.size()];
    for(int i = 0; i < category.size(); ++i)
        bound[i] = 1;
    for(int i = 0; i < points.size(); ++i)
        points[i]->topk = 0;

    int rest = category.size();
    double lambda = 0.5;
            ...
            for (int i = 0; i < points.size(); ++i)
            {
                ...
                for (int j = 0; j < category.size(); ++j)
                {
                    if (points[i]->cateAttr == category[j])
                    {
                        if (bound[j] > 0)
                        {
                            if ((1 - lambda) * u->dot_product(points[i]) + lambda > max_threshold)
                            {
                                topSet->points.push_back(points[i]);
                                is_found = true;
                                points[i]->topk = 1;
                            ...
                            {
                                topSet->points.push_back(candidate);
                                is_found = true;
                                candidate->topk = 1;
                            }
                        } else
                        {
                            if ((1 - lambda) * u->dot_product(points[i]) > max_threshold)
                            {
                                max_threshold = (1 - lambda) * u->dot_product(points[i]);
                                candidate = points[i];
                            }
                        }
                        break;
