       ...
       std::ofstream &out_cp, bool EnglishChinese)
{
    ...
    for(int i = 1; i < M; ++i)
    {
        ...
        while(current.size() > 0)
        {
            ...
            for(int j = 0; j < current.size(); j++)
            {
                ...
                //if intersecting, calculate the distance
                if(relation == 0)
                {
                    needAsk = true;
            ...
            {
                numOfQuestion++;
                point_t *p1 = realDataset->points[pset->points[i]->id];
                point_t *p2 = realDataset->points[current[p_index]->id];
    ...
    resultSet->resultEvaluation(realDataset, "RH", numOfQuestion, s, hdl, client_info, out_cp, EnglishChinese);
